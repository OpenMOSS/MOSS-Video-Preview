...
class ProcessorArguments:
    ...
    extract_frame_func: str = field(
        default="av",
        metadata={"help": "Python package for frame extraction"},
    )
    frame_extract_num_threads: int = field(
        default=1,
        metadata={"help": "The number of threads used to extract video frames. Only valid for cv2"},
    )
    video_minlen: int = field(
        default=0,
        metadata={"help": "The minimum number of sampled frames for video inputs."},
    )
...
class ModelArguments(QuantizationArguments, ProcessorArguments, ExportArguments, VllmArguments):
    ...
    mllama_use_full_attn: bool = field(
        default=False,
        metadata={"help": "Whether or not use full attention for mllama's vision encoder."},
    )
    mllama_add_video_position_encoding: bool = field(
        default=False,
        metadata={"help": "Whether or not add video_position_encoding for mllama."},
    )
    add_image_token_in_input_ids: bool = field(
        default=False,
        metadata={"help": "Whether or not add <image> in text inputs for mllama."},
    )
    mllama_pool_mode: Optional[str] = field(
        default=None,
        metadata={"help": "Pooling mode for mllama's vision encoder."},
    )
    mllama_pool_stride: Optional[int] = field(
        default=None,
        metadata={"help": "Pooling stride for mllama's vision encoder when using pooling."},
    )
