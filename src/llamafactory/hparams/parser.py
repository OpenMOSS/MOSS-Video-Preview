...
def _parse_args(parser: "HfArgumentParser", args: Optional[Dict[str, Any]] = None, parser_config_path: str = None) -> Tuple[Any]:
    if args is not None:
        return parser.parse_dict(args)

    if len(sys.argv) == 2 and sys.argv[1].endswith(".yaml"):
        return parser.parse_yaml_file(os.path.abspath(sys.argv[1]))
    elif parser_config_path is not None and parser_config_path.endswith(".yaml"):
        return parser.parse_yaml_file(os.path.abspath(parser_config_path))

    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        return parser.parse_json_file(os.path.abspath(sys.argv[1]))
    elif parser_config_path is not None and parser_config_path.endswith(".json"):
...
def _parse_train_args(args: Optional[Dict[str, Any]] = None, parser_config_path: str = None) -> _TRAIN_CLS:
    parser = HfArgumentParser(_TRAIN_ARGS)
    return _parse_args(parser, args, parser_config_path)


def _parse_infer_args(args: Optional[Dict[str, Any]] = None, parser_config_path: str = None) -> _INFER_CLS:
...
def get_train_args(args: Optional[Dict[str, Any]] = None, parser_config_path: str = None) -> _TRAIN_CLS:
    model_args, data_args, training_args, finetuning_args, generating_args = _parse_train_args(args, parser_config_path)
    ...
    if training_args.do_eval and data_args.eval_dataset is None and data_args.val_size < 1e-6:
        raise ValueError("Please specify dataset for evaluation.")
    
    if training_args.do_predict and data_args.predict_dataset is None:
        raise ValueError("Please specify dataset for prediction.")

    # Add restriction: do_predict cannot be used with do_train or do_eval
    if training_args.do_predict and (training_args.do_train or training_args.do_eval):
        raise ValueError("Cannot use `do_predict` together with `do_train` or `do_eval`. Please use them separately.")

    if training_args.predict_with_generate:
        if is_deepspeed_zero3_enabled():
            raise ValueError("`predict_with_generate` is incompatible with DeepSpeed ZeRO-3.")

        if data_args.eval_dataset is None and data_args.predict_dataset is None:
            raise ValueError("Cannot use `predict_with_generate` if both `eval_dataset` and `predict_dataset` is None.")
...
def get_infer_args(args: Optional[Dict[str, Any]] = None, parser_config_path: str = None) -> _INFER_CLS:
    model_args, data_args, finetuning_args, generating_args = _parse_infer_args(args, parser_config_path)
