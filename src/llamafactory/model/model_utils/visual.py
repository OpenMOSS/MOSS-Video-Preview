...
def get_forbidden_modules(model: "PreTrainedModel", finetuning_args: "FinetuningArguments") -> Set[str]:
    r"""
    ...
    if model_type in ["llava", "paligemma"]:
        ...
        # for mllama, this arg only determine whether projector or projector+LLM is trainable
        # - if train_mm_proj_only is True, then, LLM (text part) should be frozen
        if finetuning_args.freeze_language_model:
            # add lm_head
            forbidden_modules.add("language_model.lm_head")
            # add embedding_layer
            forbidden_modules.add("language_model.model.embed_tokens")
            # add self_attention_layers
            cross_attention_layers = model.language_model.model.cross_attention_layers
            for layer_idx in range(len(model.language_model.model.layers)):
                if layer_idx in cross_attention_layers:
                    continue
                else:
                    forbidden_modules.add("language_model.model.layers.{}".format(str(layer_idx)))
            # add norm
            forbidden_modules.add("language_model.model.norm")
            # add rotary_emb
            forbidden_modules.add("language_model.model.rotary_emb")
